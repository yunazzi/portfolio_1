$(function(){
    


    
    /*★★★★★★★★★★★*/
    /*slider*/
    main_slider1();
    function main_slider1(){
        $('#slider1 li:nth-child(1)').animate({'bottom':'600px','right':'800px', 'width':'1px','height':'auto'},0);
        $('#slider1 li:nth-child(1)').animate({'bottom':'600px','right':'800px','width':'685px','height':'auto'},1000);
        $('#slider1 li:nth-child(1)').animate({'bottom':'0px','right':'0','width':'685px','height':'auto'},3000);

        $('#slider1 li:nth-child(2)').animate({'bottom':'1000px','right':'0'},0);
        $('#slider1 li:nth-child(2)').animate({'bottom':'30px','right':'0'},2000);
        //bottom: -200px; right: 450px;
        $('#slider1 li:nth-child(3)').animate({'bottom':'-200px'},0);
        $('#slider1 li:nth-child(3)').animate({'bottom':'30px'},2000);
        //bottom: 50px; right: 100px;
        $('#slider1 li:nth-child(4)').animate({'bottom':'-200px','right':'300px'},0);
        $('#slider1 li:nth-child(4)').animate({'bottom':'50px','right':'100px'},2000);
        //top: 50px; right: 20px;
        $('#slider1 li:nth-child(5)').animate({'top':'-200px','right':'50px'},0);
        $('#slider1 li:nth-child(5)').animate({'top':'50px','right':'20px'},2000);
        //top: 30px; right: 150px;
        $('#slider1 li:nth-child(6)').animate({'top': '-30px', 'right': '50px'},0);
        $('#slider1 li:nth-child(6)').animate({'top': '30px', 'right': '150px'},2000);
        //큰글씨 left:20px; 
        $('#slider1 li:nth-child(7)').animate({'left':'-200px'},0);
        $('#slider1 li:nth-child(7)').animate({'left':'60px'},1200);
        $('#slider1 li:nth-child(7)').animate({'left':'20px'},1500);
        //작은글씨 fadeInOut
        $('#slider1 li:nth-child(8)').fadeOut(2000);
        $('#slider1 li:nth-child(8)').fadeIn(1500);
    }
    function main_slider2(){
        //top: 0; right: 40px;
        $('#slider2 li:nth-child(1)').animate({'top':'0','right':'-600px'},0);
        $('#slider2 li:nth-child(1)').animate({'top':'0','right':'40px'},2000);
        //top: 100px; right:20px;
        $('#slider2 li:nth-child(2)').animate({'top':'-500px','right':'400px'},0);
        $('#slider2 li:nth-child(2)').animate({'top':'100px','right':'20px'},1000);
        //top: 160px; right: 150px;
        $('#slider2 li:nth-child(3)').animate({'top':'-200px','right':'300px'},0);
        $('#slider2 li:nth-child(3)').animate({'top':'160px','right':'150px'},1000);
        //큰글씨
        $('#slider2 li:nth-child(4)').animate({'left':'-200px'},0);
        $('#slider2 li:nth-child(4)').animate({'left':'60px'},1200);
        $('#slider2 li:nth-child(4)').animate({'left':'20px'},1500);
        //작은글씨
        $('#slider2 li:nth-child(5)').fadeOut(2000);
        $('#slider2 li:nth-child(5)').fadeIn(1500);
    }
    function main_slider3(){
        //top: 0; left: 240px;
        $('#slider3 li:nth-child(1)').animate({'top':'-300px','left':'-100px','width':'1px','height':'auto'},0);
        $('#slider3 li:nth-child(1)').animate({'top':'0px','left':'240px','width':'685px','height':'auto'},1000);
        //top: 100px; left: 750px;
        $('#slider3 li:nth-child(2)').animate({'top':'-300px','left':'750px'},0);
        $('#slider3 li:nth-child(2)').animate({'top':'100px','left':'750px'},2000);
        //top: 120px; left: 350px;
        $('#slider3 li:nth-child(3)').animate({'top':'-400px','left':'350px'},0);
        $('#slider3 li:nth-child(3)').animate({'top':'120px','left':'350px'},2000);
        //top: 200px; left: 430px;
        $('#slider3 li:nth-child(4)').animate({'top':'-600px','left':'430px'},0);
        $('#slider3 li:nth-child(4)').animate({'top':'200px','left':'430px'},2000);
        //top: 230px; left: 370px;
        $('#slider3 li:nth-child(5)').animate({'top':'-500px','left':'-100px'},0);
        $('#slider3 li:nth-child(5)').animate({'top':'230px','left':'370px'},1000);
        //top: 120px; left: 500px;
        $('#slider3 li:nth-child(6)').animate({'top':'-700px','left':'500px'},0);
        $('#slider3 li:nth-child(6)').animate({'top':'120px','left':'500px'},2000);
        //top: 250px; left: 530px;
        $('#slider3 li:nth-child(7)').animate({'top':'-700px','left':'530px'},0);
        $('#slider3 li:nth-child(7)').animate({'top':'250px','left':'530px'},1500);
        //top: 120px; left: 665px;
        $('#slider3 li:nth-child(8)').animate({'top':'-700px','left':'665px'},0);
        $('#slider3 li:nth-child(8)').animate({'top':'120px','left':'665px'},2000);
        //top: 225px; left: 725px;
        $('#slider3 li:nth-child(9)').animate({'top':'-700px'},0);
        $('#slider3 li:nth-child(9)').animate({'top':'225px'},2000);
        //top: 150px; left: 465px;
        $('#slider3 li:nth-child(10)').animate({'top':'-700px','left':'-100px'},0);
        $('#slider3 li:nth-child(10)').animate({'top':'150px','left':'465px'},2000);
        //큰글씨
        $('#slider3 li:nth-child(11)').animate({'left':'-200px'},0);
        $('#slider3 li:nth-child(11)').animate({'left':'60px'},1200);
        $('#slider3 li:nth-child(11)').animate({'left':'20px'},1500);
        //작은글씨
        $('#slider3 li:nth-child(12)').fadeOut(2000);
        $('#slider3 li:nth-child(12)').fadeIn(1500);
    }
    var s1 =0;
    var s2 =0;
    var s3 =0;
    slider_move1();
    function slider_move1(){
        s1=1;
        s2=0;
        s3=0;
        //둥근버튼, 각페이지 left 위치값
        $('.roundBt1').addClass('on');
        $('.roundBt2').removeClass('on');
        $('.roundBt3').removeClass('on');

        $('#slider_wrap1').animate({'left':'0px'},1000);
        $('#slider_wrap2').animate({'left':'960px'},1000);
        $('#slider_wrap3').animate({'left':'1920px'},1000);
    }
    function slider_move2(){
        s1=0;
        s2=1;
        s3=0;
        //둥근버튼, 각페이지 left 위치값
        $('.roundBt2').addClass('on');
        $('.roundBt1').removeClass('on');
        $('.roundBt3').removeClass('on');

        $('#slider_wrap1').animate({'left':'-960px'},1000);
        $('#slider_wrap2').animate({'left':'0px'},1000);
        $('#slider_wrap3').animate({'left':'960px'},1000);
    }
    function slider_move3(){
        s1=0;
        s2=0;
        s3=1;
        //둥근버튼, 각페이지 left 위치값
        $('.roundBt3').addClass('on');
        $('.roundBt2').removeClass('on');
        $('.roundBt1').removeClass('on');
        
        $('#slider_wrap1').animate({'left':'-1920px'},1000);
        $('#slider_wrap2').animate({'left':'-960px'},1000);
        $('#slider_wrap3').animate({'left':'0px'},1000);
    }
    $('.roundBt1').click(function(){
        main_slider1();//애니메이션함수
        slider_move1();//둥근버튼, 위치값조절해주는 함수
    });
    $('.roundBt2').click(function(){
        main_slider2();
        slider_move2();
    });
    $('.roundBt3').click(function(){
        main_slider3();
        slider_move3();
    });
    //좌우버튼, 자동실행
    $('#button_left').click(function(){
        if(s1 == 1){
            main_slider3();
            slider_move3();
        }else if(s2 == 1){
            main_slider1();
            slider_move1();
        }else{
            main_slider2();
            slider_move2();
        }
    });
    $('#button_right').click(function(){
        if(s1 == 1){
            main_slider2();
            slider_move2();
        }else if(s2 == 1){
            main_slider3();
            slider_move3();
        }else{
            main_slider1();
            slider_move1();
        }
    });
    function autoslider(){
        $('#button_right').trigger('click');//강제실행해라
    }
    setInterval(autoslider,6000);
    /*★★★★★★★★★★★*/
    //gnb
    $('#subMenu').slideUp(0);
    
    $('ul#gnb').click(function(){
        $('#subMenu').slideDown();
    });
    $('#subMenu p').click(function(){
        $('#subMenu').slideUp();
    });
    
    $('#subMenu').mouseleave(function(){
        $('#subMenu').slideUp();
    });


    $('.notice').fadeIn(0);
    $('.lecture').fadeOut(0);

    $('.noticeBt').click(function(){
        $('.notice').fadeIn(300);
        $('.lecture').fadeOut(300);
        
    });
    $('.lectureBt').click(function(){
        $('.lecture').fadeIn(300);
        $('.notice').fadeOut(300);
    });

    var noticeBt_autoPlay = setInterval(autonotice,2000);
    var lectureBt_autoPlay = setInterval(autolecture,4000);
    //clear에서 써먹어야되서 참조함수넣음
    
    function noticeBt_ClearPlay(){
        clearInterval(noticeBt_autoPlay);
        clearInterval(lectureBt_autoPlay);
    }
    function autonotice(){
        $('.noticeBt').trigger('click');//3초
        //noticeBt 클릭을 강제 실행
    }
    function autolecture(){
        $('.lectureBt').trigger('click');//6초
        //lectureBt 클릭을 강제 실행
    }

    //클릭을 안한상태(play모드)
    var i=0;
    
    $('.stopPlay').click(function(){
        if(i == 0){
            //버튼모양이 [ㅁ]인 상태
            $('.stopPlay').css('backgroundPosition','right top');
            noticeBt_ClearPlay();
            //[>]버튼으로 변경
            i=1;
        }else if(i==1){
            $('.stopPlay').css('backgroundPosition','left top');
            
            noticeBt_autoPlay = setInterval(autonotice,2000);
            lectureBt_autoPlay = setInterval(autolecture,4000);
            //[ㅁ]버튼으로 변경
            i=0;
        }
    });

    $('.gallery1').click(function(){
        $('.container_hidden1').slideDown(600);
    });
    $('.gallery2').click(function(){
        $('.container_hidden2').slideDown(600);
    });



});
